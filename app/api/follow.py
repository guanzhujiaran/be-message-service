...
async def get_relation(
    session: SessionDep,
    user: CurrentUser,
    # 注意：`StrInt` 必须走 `Annotated[...]`，写成 `x: StrInt = Query(...)` 时
    # FastAPI 会丢掉 BeforeValidator，字符串入参会原样透传（见 str_int 模块说明）
    target_mid: Annotated[StrInt, Query(..., description="目标用户 mid")],
